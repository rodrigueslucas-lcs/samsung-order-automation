...
function validateMxQstCoverage() {
  ...
  for (const id of officialIds) {
    ...
    if (!current) continue;
    if (!current.title || !String(current.title).trim()) errors.push(`${id}: official title is required`);
    if (!STORES.includes(current.store)) errors.push(`${id}: invalid store ${current.store || "<missing>"}`);
    if (!FEATURES.includes(current.feature)) errors.push(`${id}: invalid feature ${current.feature || "<missing>"}`);
    ...
    if (current.coverage !== "missing" && (!current.spec || !current.test)) {
  ...
  if (classifiedTotal !== officialIds.length) errors.push(`MX classified total: expected ${officialIds.length}, found ${classifiedTotal}`);
  if (errors.length) throw new Error(`Invalid MX QST coverage mapping:\n${errors.join("\n")}`);

  return { market: "MX", officialTotal: officialIds.length, ...counts };
}

module.exports = { COVERAGE_STATES, FEATURES, STORES, validateMxQstCoverage };
