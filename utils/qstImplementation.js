...
function marketQstRoot(code) {
  const key = code.toLowerCase();
  if (TEST_PATHS[key]?.qst) return resolveTestPath(TEST_PATHS[key].qst);
  // CL/CO are roadmap markets and may not have a physical implementation yet.
  return resolveTestPath(path.join('tests/markets', key, 'qst'));
}

function collectMarketImplementation(market) {
  ...
  const cases = [];
  for (const entry of collectTitlesFromFiles(walkSpecFiles(marketQstRoot(code)))) {
    for (const id of officialIdsFromTitle(entry.title)) {
      cases.push({ ...entry, id, source: 'market' });
    }
  }

  const sharedRoot = resolveTestPath(TEST_PATHS.shared.smbQst);
...
function validateSharedSpecMarketTags() {
  const errors = [];
  const sharedRoot = resolveTestPath(TEST_PATHS.shared.smbQst);
  for (const entry of collectTitlesFromFiles(walkSpecFiles(sharedRoot))) {
    ...
    const tags = marketTagsFromTitle(entry.title);
    if (tags.length !== 1) {
      errors.push(`${entry.spec}: official shared test title must contain exactly one market tag; found ${tags.join(', ') || 'none'}.`);
    }
  }
  return errors;
}

function validateOfficialImplementation() {
...
module.exports = {
  ...
  testTitles,
  validateOfficialImplementation,
  validateSharedSpecMarketTags,
  walkSpecFiles,
};
