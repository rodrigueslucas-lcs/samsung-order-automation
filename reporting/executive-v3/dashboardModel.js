const smb = require('../../governance/smb-qst.json');
...
const MX_COVERAGE_OVERRIDES = Object.freeze({
  'SAM-24962': { coverage: 'full', spec: 'tests/markets/mx/qst/base-store/authenticated-safe.spec.js', test: 'SAM-24962 - Login Home page', notes: 'Registered session lands on the MX storefront home and proves My Profile is available.' },
  'SAM-24963': { coverage: 'full', spec: 'tests/markets/mx/qst/base-store/authenticated-safe.spec.js', test: 'SAM-24963 - Validate My account menu', notes: 'My Account menu options are explicitly validated in the authenticated flow.' },
  ...
  'SAM-24969': { coverage: 'full', spec: 'tests/markets/mx/qst/base-store/bc-add-safe.spec.js', test: 'SAM-24969 - Add product from BC page', notes: 'PreQA2 BC catalog -> PDP -> Add to Cart flow is implemented and has runtime proof of the cart mutation. A later PDP-return timeout is an execution-stability issue, not missing implementation.' },
  'SAM-24971': { coverage: 'partial', spec: 'tests/markets/mx/qst/base-store/safe-reuse.spec.js', test: 'SAM-24971 - Cart page UI', notes: 'Core cart UI/product/summary assertions are implemented; remaining official UI detail keeps this Partial.' },
  'SAM-24972': { coverage: 'full', spec: 'tests/markets/mx/qst/base-store/safe-reuse.spec.js', test: 'SAM-24972 - Increase decrease and delete cart quantity', notes: 'Quantity increase/decrease/delete and empty-cart confirmation are implemented.' },
  'SAM-24975': { coverage: 'partial', spec: 'tests/markets/mx/qst/base-store/rewards-safe.spec.js', test: 'SAM-24975 - Rewards text on cart checkout and payment', notes: 'Rewards text is validated on Cart, Checkout and Payment. Tooltip interaction is still a documented remaining assertion.' },
  ...
  'SAM-24982': { coverage: 'partial', spec: 'tests/markets/mx/qst/base-store/tradeup-safe.spec.js', test: 'SAM-24982 - Trade-up interaction preserves controlled cart', notes: 'Trade-up interaction is automated; complete official resulting-cart/delete behavior remains to be proven.' },
  'SAM-24985': { coverage: 'full', spec: 'tests/markets/mx/qst/base-store/extended-warranty-safe.spec.js', test: 'SAM-24985 - Add Extended Warranty on Cart', notes: 'Runtime-proven on S2 with the eligible PDP SKU and Service Pack association reflected in cart.' },
  'SAM-24986': { coverage: 'partial', spec: 'tests/markets/mx/qst/base-store/cart-isolation-safe.spec.js', test: 'SAM-24986 - Cart is isolated from a second account', notes: 'Independent second-account cart isolation is automated. Explicit logout semantics from the official wording remain a coverage refinement.' },
  'SAM-24988': { coverage: 'full', spec: 'tests/markets/mx/qst/base-store/safe-reuse.spec.js', test: 'SAM-24988 - Checkout button on cart page', notes: 'Controlled cart Checkout navigation is explicitly proven.' },
  'SAM-24989': { coverage: 'partial', spec: 'tests/markets/mx/qst/base-store/safe-reuse.spec.js', test: 'SAM-24989 - Order Summary on checkout page', notes: 'Order summary values are automated; deeper official presentation details remain partial.' },
  'SAM-24990': { coverage: 'full', spec: 'tests/markets/mx/qst/base-store/safe-reuse.spec.js', test: 'SAM-24990 - Contact information', notes: 'Mandatory guest contact information is filled and accepted before Delivery.' },
  ...
  'SAM-24992': { coverage: 'full', spec: 'tests/markets/mx/qst/base-store/registered-address-safe.spec.js', test: 'SAM-24992 - Select saved address', notes: 'Saved-address selection and checked state are explicitly validated when the authenticated test account has prerequisite data.' },
  'SAM-24993': { coverage: 'full', spec: 'tests/markets/mx/qst/base-store/registered-address-safe.spec.js', test: 'SAM-24993 - Save shipping and billing address option is available', notes: 'Runtime-proven registered-user save-address option behavior, including the S2 default-selected state.' },
  'SAM-24994': { coverage: 'full', spec: 'tests/markets/mx/qst/base-store/registered-address-safe.spec.js', test: 'SAM-24994 - Checkout accepts a new unsaved address', notes: 'Runtime-proven registered checkout with a new address while explicitly keeping the address unsaved.' },
  'SAM-24995': { coverage: 'full', notes: 'Guest checkout explicitly proves the Save option is not exposed.' },
  'SAM-24999': { coverage: 'full', spec: 'tests/markets/mx/qst/base-store/address-negative-safe.spec.js', test: 'SAM-24999 - Invalid address is rejected', notes: 'Invalid postal code produces an observable validation barrier and prevents normal delivery progression.' },
  'SAM-25000': { coverage: 'partial', spec: 'tests/markets/mx/qst/base-store/registered-address-safe.spec.js', test: 'SAM-25000 - Switch saved and new address modes', notes: 'Saved/new address switching is automated. Delivery-mode switching remains a separate official path to prove.' },
  'SAM-25001': { coverage: 'full', spec: 'tests/markets/mx/qst/base-store/back-to-top-safe.spec.js', test: 'SAM-25001 - Back to Top returns the cart to the top', notes: 'Dedicated Back-to-Top control visibility and viewport return are explicitly validated.' },
  'SAM-25002': { coverage: 'full', spec: 'tests/markets/mx/qst/base-store/registered-order.spec.js', test: 'SAM-25002 - Complete card payment with registered user', notes: 'Runtime-proven registered Mercado Pago card payment through Samsung checkout to order confirmation.' },
  'SAM-25004': { coverage: 'partial', spec: 'tests/markets/mx/qst/base-store/payment-modes-safe.spec.js', test: 'SAM-25004 - PayPal payment mode is available and selectable', notes: 'Real PayPal option availability/selection is automated safely; order submission is intentionally not claimed.' },
  'SAM-25005': { coverage: 'partial', spec: 'tests/markets/mx/qst/base-store/payment-modes-safe.spec.js', test: 'SAM-25005 - Pay in Cash payment mode is available and selectable', notes: 'Real Pay in Cash option availability/selection is automated safely; order submission is intentionally not claimed.' },
  'SAM-25006': { coverage: 'partial', spec: 'tests/markets/mx/qst/base-store/rewards-payment-registered-safe.spec.js', test: 'SAM-25006 - Rewards payment mode is available and selectable', notes: 'Historical implementation retained for traceability only. SAM-25006 is excluded from the active MX QST runner after the 2026-09-24 PSE/Colombia clarification.' },
  'SAM-25010': { coverage: 'partial', spec: 'tests/markets/mx/qst/base-store/tracking-safe.spec.js', test: 'SAM-25010 - Track Order with email and Order ID', notes: 'Automation creates/reuses a causal guest order, requests OTP, reads the verification email and submits the code. S2 has also reproduced an order-not-found behavior after OTP; runtime result remains separate.' },
  'SAM-25011': { coverage: 'full', spec: 'tests/markets/mx/qst/base-store/backoffice.spec.js', test: 'SAM-25011 - BackOffice order and product basic advanced search', notes: 'Runtime-proven S2 BackOffice validation covers order/product search in both basic and advanced search modes when the environment credential is provisioned.' },
  'SAM-25016': { coverage: 'partial', spec: 'tests/markets/mx/qst/base-store/mobile-sticky.spec.js', test: 'SAM-25016 - Mobile Sticky checkout', notes: 'Sticky mobile cart checkout is proven; sticky behavior deeper in checkout remains a documented refinement.' },
});
