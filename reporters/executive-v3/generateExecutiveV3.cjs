...
function render(model) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Samsung SMB QA Automation</title><link rel="stylesheet" href="dashboard.css"></head><body><header class="hero"><div class="hero-row"><div><div class="eyebrow">Samsung · Quality Engineering · SMB</div><h1>QA Automation Control Center</h1><p>${headline}</p></div><div class="health ${health.toLowerCase().replace(/\s+/g,'-')}"><span class="eyebrow">Current build health</span><strong>${esc(health)}</strong></div></div><div class="meta"><span>Generated ${esc(model.generatedAt)}</span><span>Official scope 362</span><span>P1/QST 144</span><span>P2/DST 218</span><span>Data audit ${model.audit.ok ? 'OK' : 'CHECK'}</span></div></header><main>
<section class="panel runtime-banner"><div class="panel-title"><div><h2>Current Build Runtime</h2><p>Authoritative Playwright result for this execution.</p></div><span class="tag">LIVE EXECUTION</span></div>${renderExecution(execution)}</section>
<section class="panel attention-panel"><div class="panel-title"><div><h2>Needs Attention</h2><p>Only failures and known blockers from the current build.</p></div><span class="tag">${runtime ? runtime.failed + runtime.blocked : 0} ITEMS</span></div>${renderAttention(execution)}</section>
${detail('Official SMB Scope','Current P1/P2 priority model across LATAM.','362 SCENARIOS',scopeReference,true)}
${detail('MX Automation Coverage','Implementation maturity for the preserved MX case mapping; separate from runtime.','LEGACY 37-ID MAP',`<div class="coverage-row"><div class="coverage-box"><small>FULL</small><b>${mx.full ?? 'N/A'}</b></div><div class="coverage-box"><small>PARTIAL</small><b>${mx.partial ?? 'N/A'}</b></div><div class="coverage-box"><small>MISSING</small><b>${mx.missing ?? 'N/A'}</b></div><div class="coverage-box"><small>FULL COVERAGE</small><b>${pct(mx.fullPercent)}</b></div></div><div class="scope-note">MX automation coverage contains 37 TCs in the preserved implementation mapping. It is retained for traceability; it is not the current official MX P1 denominator (38) and not the current Base Store runner selection (30).</div>`)}
