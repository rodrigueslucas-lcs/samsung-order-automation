...
const defaultLedger = require("../preqa2-validation.json");
const {
...
const CURRENT_PARTIAL_FIXTURE = "SAM-24968";
...
test("official PASS remains PASS even when automation coverage is still a gap", () => {
  ...
  sourceLedger.markets.MX.results[CURRENT_PARTIAL_FIXTURE] = mxPass();
  const mx = getMxPreqa2PromotionPlan({ sourceLedger });
  assert.equal(mx.validationPassAutomationGap.length, 1);
  assert.equal(mx.validationPassAutomationGap[0].id, CURRENT_PARTIAL_FIXTURE);
...
test("automation coverage review requires explicit implemented-proven evidence", () => {
  const sourceLedger = emptyLedger();
  sourceLedger.markets.MX.status = "ACTIVE";
  sourceLedger.markets.MX.results[CURRENT_PARTIAL_FIXTURE] = mxPass({ automation: "implemented-proven" });
  const mx = getMxPreqa2PromotionPlan({ sourceLedger });
  assert.equal(mx.coverageReviewCandidates.length, 1);
  assert.equal(mx.coverageReviewCandidates[0].id, CURRENT_PARTIAL_FIXTURE);
