...
function resolveRuntimeCredentials() {
  ...
  const email = process.env.MX_SAMSUNG_EMAIL?.trim() || local.email;
  const password = process.env.MX_SAMSUNG_PASSWORD || local.password;
  if (!email || !password) {
    throw new Error(
...
async function openMxHome(page, context) {
  ...
  if (renderedBeforeNavigation) {
    console.log("[auth:login:mx] reusing an already rendered MX S1 storefront tab");
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    if (renderedSibling) {
      console.log("[auth:login:mx] current MX tab is blank; switching to a rendered sibling MX tab");
      return renderedSibling;
    }
    if (attempt < 3) await page.waitForTimeout(3000);
  }
  throw new Error("MX S1 storefront did not render My Profile after 3 navigation attempts and no rendered sibling tab was available.");
}
...
async function waitForStorefrontOrVerification(page) {
  ...
  if (outcome !== "storefront") throw new Error("Samsung Account did not return to the MX S1 storefront within the allowed time.");
}
...
async function loginMxSamsungAccount() {
  ...
  let page = existingAccountPage || await findRenderedMxPage(context) || await context.newPage();
