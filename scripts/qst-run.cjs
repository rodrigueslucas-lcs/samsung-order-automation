...
const { TEST_PATHS } = require("../config/testPaths.cjs");
...
if (!validTypes.has(type)) throw new Error(`Unsupported QST type: ${type}`);
if (store && !validStores.has(store)) throw new Error(`Unsupported QST store: ${store}`);

const testPath = store === "base-store"
  ? TEST_PATHS.legacy.peQstBaseStore
  : store === "epp"
    ? TEST_PATHS.legacy.peQstEpp
    : TEST_PATHS.legacy.peQst;
