...
function getChromeLaunch() {
  ...
  if (process.platform === "win32") {
    ...
    if (!command) throw new Error(`Google Chrome was not found. Checked: ${candidates.join(", ")}`);
    return { command, args: chromeArguments };
  }
  const candidates = ["google-chrome", "google-chrome-stable", "chromium", "chromium-browser"];
  const command = candidates.find((candidate) => spawnSync("which", [candidate], { stdio: "ignore" }).status === 0);
  if (!command) throw new Error(`Chrome/Chromium was not found. Checked: ${candidates.join(", ")}`);
  return { command, args: chromeArguments };
}
...
chrome.once("spawn", () => {
  chrome.unref();
  console.log(`A maximized Chrome window was opened with the dedicated ${config.environment} PE QA profile.`);
  console.log("");
  ...
  console.log(`7. Run: PE_QST_ENVIRONMENT=${config.environment} npm run auth:export:pe`);
});
