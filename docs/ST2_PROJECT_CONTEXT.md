...
- Manual validation confirmed that ST2 My Account/Profile navigation redirects to `https://shop.samsung.com/` Production. TC7-TC12 and TC40 are Not Applicable in ST2 and are statically skipped before navigation. Never use the Production destination to validate staging data.
- Authenticated Cart → Checkout correctly bypasses Guest Login and reaches Delivery. TC39 is Automated with a saved-address selection and a pre-submit stop at Payment. TC40 is Not Applicable because its required Profile readback redirects to Production.
...
- Authenticated new-address checkout can reuse the proven guest Delivery baseline and stop before Place Order. Profile-dependent mutations/readback remain disabled because they would leave ST2 for Production.
- TC39 passed with a renewed reusable auth state. The test selected `Dirección guardada`, required a real saved-address radio to be checked, reused the guest shipping baseline and stopped at Payment. TC40 is Not Applicable because its required Profile verification redirects to Production.
...
- Manual validation confirmed that ST2 My Account/Profile links redirect to Production. TC7-TC12 and TC40 are Not Applicable in ST2; their specs must remain skipped and automation must never follow those links or mutate Production data. TC6 remains valid because it asserts only the authenticated menu structure.
...
- TC12 with an existing order: currently disabled/Not Applicable because ST2 My Orders redirects to Production; do not run against Production.
