...
- Authenticated address inspection on 2026-08-21 found no safe Profile route: the visible `[role=menuitem][data-an-la="user name"]` could be resolved but a normal click timed out, and the authenticated menu exposed no address link. Keep TC7 and TC8 Blocked until the current ST2 Profile/address entry point renders and works reliably; do not navigate directly to an inferred account route or modify pre-existing account data.
