...
Official total: `31 + 1 + 5 + 2 + 44 = 83`. Supplementary tests do not change this count.
...
| C — relevant | Authenticated saved-address checkout | 39 | Reuse the authenticated setup and guest Delivery→Payment methods; requires a fresh local auth snapshot and a safe saved QA address. |
...
| 39 | Use saved address in checkout | 5 | `tests/st2/base-store/checkout/authenticated-checkout.spec.js` | `CartPage`, `CheckoutPage`, `PaymentPage` | Auth preflight, authenticated Cart setup, `fillCustomerData()`, `selectShippingMethod()`, `acceptTerms()`, `continueToPayment()`, `validatePaymentPage()` | `selectSavedAddressAndValidate()`; do not call `validateAuthenticatedDeliveryForm()` because it intentionally switches to Nueva dirección | Fresh ignored auth state; existing disposable QA saved address | Delivery region → tabpanel `Envío` → `getByRole('radio', {name:'Dirección guardada', exact:true})`; click associated visible label only if needed, then assert checked and address card visible | `auth:verify` → setup/add/cart → authenticated checkout → Contact Info → Dirección guardada → reuse guest shipping/terms/continue → Payment | Payment visible and selected saved address represented in Delivery/Payment summary; no Place Order | `npx playwright test tests/st2/base-store/checkout/authenticated-checkout.spec.js --project=chromium --grep "TC39" --workers=1` |
