...
`tests/s2/pe/dst/epp/` contains guarded thin EPP wrappers. Search plus the seven low/medium-risk Base Store domains now use the country-scoped target convention; checkout, payment and BackOffice remain incremental.
...
5. ✅ Move Home, PDP, Cart, Guest Login, Profile, Mobile and Smoke by logical area; keep checkout/payment incremental and move BackOffice last because its environment commands and discovery documentation are widely referenced.
6. Remove compatibility paths only after all consumers use the country-scoped `tests/s2/pe/dst` paths.

Search, Home, PDP, Cart, Guest Login, Profile, Mobile, Smoke and EPP now use the country-scoped target layout. Checkout, payment and BackOffice remain on legacy `tests/st2` paths and will continue incrementally.
