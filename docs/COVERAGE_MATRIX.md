...
| 59 | Customer Order Journey | Customer able to place one IM order with Trade-in and SC+. (If there is available data) | ⚠️ Partial | `tests/st2/base-store/smoke/guest-checkout.spec.js` | Automation and manual validation confirmed that ST2 removes SC+ at Checkout with the unavailable/stock message and keeps the user in Cart; blocker is current test mass/configuration and no order was created |
