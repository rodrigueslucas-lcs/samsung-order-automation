...
test("runtime guard accepts approved market route", () => {
  const url = assertApprovedPreqa2Url(
    "https://p6-pre-qa2.samsung.com/mx/smartphones/all-smartphones/",
    { market: "MX" }
  );
  assert.equal(url.pathname, "/mx/smartphones/all-smartphones/");
});

test("runtime guard rejects Production and foreign hosts", () => {
  assert.throws(
